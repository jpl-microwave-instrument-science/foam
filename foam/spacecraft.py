...
class spacecraft(): 
    ...
    def write_radiometer_ck(self, look_angle, look_axis, rpm, scan_axis, antenna_fov_angle=None, antenna_pattern=None):
        """ Writes the camera kernel for a given radiometer. 
            ...
            :param scan_axis: Scan rotation axis
                              Either 'X', 'Y', 'Z' in spacecraft frame or a vector
                              A negative sign can also be added e.g. -X
            :param antenna_fov_angle: Angular full width at half maximum for the antenna pattern
        ...
        file_fk.write("""KPL/FK 
                    ...
                    CK_{0}_SCLK             = {2}
                    CK_{0}_SPK              = {2}
                    """.format(inst_id, count, self.naif_id, self.sc_number))    
        file_fk.close()
        ...
        spice.ckw02(file_ck, enc_start, enc_stop, inst_id, 
        ...
        # Make instrument kernel with field of view information
        if antenna_fov_angle is None: 
            antenna_fov_angle = 180.
